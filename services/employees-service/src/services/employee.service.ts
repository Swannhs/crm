...
export class EmployeeService {
  ...
  async getEmployeesWithScheduleSummary(orgId: string, filters: any) {
    try {
      ...
      const shifts = await shiftRepo.findManyForEmployees(orgId, employees.map((employee: any) => employee.id), start, end);
      ...
      return employees
        .map((employee: any) => {
          ...
          const fullName =
            String(metadata.fullName || metadata.name || "").trim() ||
