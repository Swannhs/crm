...
export function ProjectDetailsView({ id }: Props) {
  ...
  const { data: columnsData, isLoading: columnsLoading } = useQuery({
    queryKey: ['board-columns', activeBoard?._id],
    queryFn: () => projectService.getColumns(activeBoard?._id),
    enabled: !!activeBoard?._id,
  });

  const { data: cardsData, isLoading: cardsLoading } = useQuery({
    queryKey: ['board-cards', activeBoard?._id],
    queryFn: () => projectService.getCards(activeBoard?._id),
    enabled: !!activeBoard?._id,
  });
  ...
  return (
    <DashboardContent maxWidth={false}>
      <Box sx={{ mb: 5, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <Box>
          <Typography variant="h4">{project?.title || 'Project Details'}</Typography>
      ...
      <Scrollbar sx={{ pb: 3 }}>
        <Stack direction="row" spacing={3} sx={{ minHeight: '70vh', alignItems: 'flex-start' }}>
          {columns.map((column: any) => (
            <Box key={column._id} sx={{ width: 320, flexShrink: 0 }}>
              <Stack
                direction="row"
                alignItems="center"
              ...
              >
                ...
                <Typography variant="caption" sx={{ px: 1, py: 0.5, borderRadius: 1, bgcolor: 'action.selected' }}>
                  {cards.filter((c: any) => c.columnId === column._id).length}
                </Typography>
              </Stack>
              ...
              <Stack spacing={2}>
                {cards
                  ...
                  .map((card: any) => (
                    <Card key={card._id} sx={{ p: 2, cursor: 'pointer', '&:hover': { boxShadow: (theme) => theme.customShadows.z12 } }}>
                ...
                <Button fullWidth variant="soft" color="inherit" startIcon={<Iconify icon="mingcute:add-line" />}>
                   Add Task
                </Button>
              </Stack>
            </Box>
